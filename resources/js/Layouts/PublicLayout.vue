...
<div
  :class="mobileMenuOpen 
    ? 'block absolute top-full right-0 w-full bg-white shadow-md md:static md:block' 
    : 'hidden md:block'"
...
>
  ...
  <div class="block md:hidden">
    <button
      ...
      class="w-full py-3 px-3 flex gap-2  hover:text-[#D4AF37]"
    >
      Products
    ...
    >
      <li
        v-for="cat in categories"
        :key="cat"
        class="block px-4 py-2 text-gray-500 hover:bg-gray-100 cursor-pointer rounded"
        @click="$inertia.get(`/allProducts?category=${cat}`)"
      >
        {{ cat }}
      </li>
          ...
          <li><Link href=""  class="py-6 w-full md:hidden flex justify-center items-center gap-4 hover:text-[#D4AF37] hover:border-b-2 hover:border-[#D4AF37] "><button class="w-1/2 bg-[#D4AF37] rounded-2xl py-2">Sign Up</button><button class="w-1/2 bg-[#D4AF37] rounded-2xl py-2">Login</button></Link></li>
        </ul>
...
<!-- CENTER: SEARCH BAR -->
    ...
    <input
        ...
        class="px-3 py-1 md:py-2 border border-[#D4AF37] md:rounded-lg rounded-2xl focus:outline-none focus:ring w-full md:w-64"
    />
